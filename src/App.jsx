            ...
            <div key={m.id} style={{background:C.white,borderRadius:14,padding:"14px 16px",border:`1.5px solid ${C.border}`,boxShadow:`0 2px 8px ${C.pink1}`,display:"flex",gap:12,alignItems:"flex-start"}}>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:10,color:C.sub,marginTop:6}}>
                  {new Date(m.createdAt).toLocaleString("ko-KR",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"})}
                </div>
              </div>
              <button onClick={()=>deleteMemo(m.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:16,color:C.sub,opacity:.5,flexShrink:0,padding:"2px 4px",lineHeight:1}} title="삭제">🗑️</button>
            </div>
          ))}
        </div>
      </div>
